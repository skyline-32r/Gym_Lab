...
export default function Edit() {
  ...
  useEffect(() => {
    if (router.isReady) {
      ...
      fetch(`http://localhost:3002/forum/edit?post_id=${postId}`)
        .then((res) => res.json())
        .then((output) => {
          setAllData(output.data[0])
          console.log(output)
          setFormData(output.data[0])
        })
    }
  }, [router.isReady])
  ...
  const handleSubmit = async (postId) => {
    ...
    await fetch(`http://localhost:3002/forum/edit`, {
      method: 'PUT',
      ...
      >
        ...
        >
          ...
          >
            ...
            <Box
              onClick={() => {
                handleSubmit(router.query.post_id)
              }}
            >
              <BtnStyles>
                編輯完成
